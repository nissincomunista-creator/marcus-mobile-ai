...
export function auditOfficialArea(input: {text:string;title:string;propertyType:string;url:string;structuredSizes?:number[];extractedValue?:number}): AreaAudit {
 ...
 const label='(?:[aá]rea\\s+(?:privativa|[uú]til|constru[ií]da|edificada|total|comum|do\\s+terreno|de\\s+terreno)|metragem\\s+constru[ií]da)';
 const amount='(\\d+(?:[.,]\\d+)*)';
 ...
 for(const m of text.matchAll(new RegExp('\\b(privativa|priv\\.|constru[ií]da|constr\\.|edificada|terreno|terr\\.)\\s*(?:estimada|aproximada|de)?\\s*[:=]?\\s*'+amount+'\\s*('+unit+')','gi')))add(numeric(m[2])*(/ha|hectare/i.test(m[3])?10000:1),/^priv/i.test(m[1])?'private':/^terr/i.test(m[1])?'land':'built',m[0]);
