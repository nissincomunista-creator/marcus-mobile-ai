...
export default function App() {
  ...
  const [selectedOriginFilter, setSelectedOriginFilter] = useState<'caixa' | 'caixa_radar' | 'judicial' | 'extrajudicial' | 'portal'>('caixa_radar');
            ...
            {activeTab === 'garimpo' && (
              ...
              >
                {!activeSelectedAuction ? (
                  <>
                    {/* The State Selection Bar at the top of the page, below header/profile button */}
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 bg-indigo-950/40 text-indigo-400 border border-indigo-900/30 rounded-lg flex items-center justify-center font-bold font-mono">
                          {selectedStateFilter || '-'}
                        </div>
                        <div>
                          <span className="text-[10px] uppercase font-bold text-slate-400 block font-mono tracking-wider">Filtrar por Estado (UF):</span>
                          <h2 className="text-sm font-bold text-slate-100">
                            {selectedStateFilter 
                              ? `Garimpando em: ${BRAZIL_STATES.find(s => s.value === selectedStateFilter)?.label || selectedStateFilter}`
                              : 'Selecione um Estado para iniciar a busca'}
                          </h2>
                        </div>
                      </div>
                      <div className="flex flex-wrap items-center gap-2">
                        {['RJ', 'SP', 'MG'].map((uf) => (
                          <button
                            key={uf}
                            onClick={() => {
                              setSelectedStateFilter(uf);
                              setSelectedCityFilter(''); // reset city filter
                              setSelectedNeighborhoodFilter(''); // reset neighborhood filter on state change
                            }}
                            className={`px-4 py-2 rounded-lg font-bold text-xs transition-all border cursor-pointer ${
                              selectedStateFilter === uf
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setSelectedCityFilter(''); // reset city filter
                            setSelectedNeighborhoodFilter(''); // reset neighborhood filter
                          }}
                          className="bg-slate-950 border border-slate-800 text-xs font-semibold px-3 py-2 rounded-lg text-slate-200 outline-none cursor-pointer hover:bg-slate-900 transition-colors"
                        >
                          <option value="" className="bg-slate-950 text-slate-200">Outro Estado...</option>
                          {BRAZIL_STATES.filter(st => !['RJ', 'SP', 'MG'].includes(st.value)).map((st) => (
                            <option key={st.value} value={st.value} className="bg-slate-950 text-slate-200">{st.value} - {st.label}</option>
                          ))}
                        </select>
                        {(selectedStateFilter || auctions.length > 0) && (
                          <button
                            onClick={handleClearAllAuctions}
                            className="text-xs text-rose-400 hover:text-rose-350 font-bold px-3 py-1.5 rounded-lg hover:bg-rose-950/20 transition-colors cursor-pointer border border-rose-900/30 shadow-sm flex items-center space-x-1"
                    ...
                    {/* Below the State Selector: Render Dashboard ONLY if state is selected */}
                    {selectedStateFilter === '' ? (
                      /* Blank placeholder state */
                      <div className="py-24 text-center text-gray-400 bg-white border border-dashed border-[#E2E8F0] rounded-xl shadow-xs">
                        <Building2 className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                        <p className="text-sm font-medium">Aguardando seleção de estado no filtro acima...</p>
                      </div>
                    ) : (
                      <Dashboard
                        auctions={auctions}
