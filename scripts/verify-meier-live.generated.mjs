...
${extracted}`;
    ...
    const sizeMatch = combinedText.match(/[aá]rea\s+(?:privativa(?:\s*\/\s*edificada)?|[uú]til|constru[ií]da)\s*:?\s*(\d+(?:[.,]\d+)?)\s*m[²2]/i) || combinedText.match(/(\d+(?:[.,]\d+)?)\s*m[²2]\s*(?:de\s+)?[aá]rea\s+privativa/i) || combinedText.match(/(?:metragem|[aá]rea\s+do\s+im[oó]vel)\s*:?\s*(\d+(?:[.,]\d+)?)\s*m[²2]/i);
    const structuredSize = detailData.structuredSizes.find((value) => value >= 10 && value <= 5e3) || 0;
    const detailedSize = (sizeMatch ? Number(sizeMatch[1].replace(",", ".")) : 0) || structuredSize;
...
var property = store.auctions.find((a) => a.auctionLink?.includes("x128862"));
assert(property);
...
try {
  ...
  assert.equal(result.auctionPrice, 301e3);
