...
// auctionPipeline/quarantine.ts
var import_node_fs = __toESM(require("node:fs"), 1);
var import_node_path = __toESM(require("node:path"), 1);
var import_node_crypto = require("node:crypto");
var root = import_node_path.default.resolve("sync-audits/quarentena_extracao");
function quarantine(draft, reasons, raw) {
  import_node_fs.default.mkdirSync(root, { recursive: true });
  const key = (0, import_node_crypto.createHash)("sha256").update(draft.auctionLink || JSON.stringify(draft)).digest("hex");
  const htmlPath = import_node_path.default.join(root, key + ".html");
  if (raw) import_node_fs.default.writeFileSync(htmlPath, raw);
  const record = { key, status: "quarentena_extracao", checkedAt: (/* @__PURE__ */ new Date()).toISOString(), reasons, htmlPath: import_node_fs.default.existsSync(htmlPath) ? htmlPath : void 0, draft };
  const target = import_node_path.default.join(root, key + ".json");
  import_node_fs.default.writeFileSync(target + ".tmp", JSON.stringify(record, null, 2));
  import_node_fs.default.renameSync(target + ".tmp", target);
  return record;
}
function preserveSource(url, raw) {
  const dir = import_node_path.default.resolve("sync-audits/source-evidence");
  import_node_fs.default.mkdirSync(dir, { recursive: true });
  const name = (0, import_node_crypto.createHash)("sha256").update(url).digest("hex") + ".html";
  const file2 = import_node_path.default.join(dir, name);
  import_node_fs.default.writeFileSync(file2, raw);
  return file2;
}
function resolveQuarantine(url) {
  const key = (0, import_node_crypto.createHash)("sha256").update(url).digest("hex"), file2 = import_node_path.default.join(root, key + ".json");
  if (import_node_fs.default.existsSync(file2)) {
    const row = JSON.parse(import_node_fs.default.readFileSync(file2, "utf8"));
    row.status = "resolvido";
    row.resolvedAt = (/* @__PURE__ */ new Date()).toISOString();
    import_node_fs.default.writeFileSync(file2 + ".tmp", JSON.stringify(row, null, 2));
    import_node_fs.default.renameSync(file2 + ".tmp", file2);
  }
}

// auctionPipeline/validation.ts
var import_node_fs3 = __toESM(require("node:fs"), 1);
var import_cheerio = require("cheerio");

// auctionPipeline/location.ts
var import_node_fs2 = __toESM(require("node:fs"), 1);

// src/utils/geoCoords.ts
var RIO_DE_JANEIRO_BAIRROS = {
  // Zona Sul
  "copacabana": [-22.9698, -43.1864],
  "leme": [-22.962, -43.167],
  "ipanema": [-22.9836, -43.2045],
  "leblon": [-22.9856, -43.223],
  "gavea": [-22.979, -43.234],
  "sao conrado": [-22.992, -43.268],
  "vidigal": [-22.993, -43.242],
  "rocinha": [-22.988, -43.248],
  "jardim botanico": [-22.968, -43.226],
  "lagoa": [-22.972, -43.207],
  "humaita": [-22.9575, -43.2],
  "botafogo": [-22.9519, -43.1856],
  "urca": [-22.9554, -43.1647],
  "flamengo": [-22.9376, -43.1764],
  "catete": [-22.926, -43.1795],
  "gloria": [-22.921, -43.177],
  "laranjeiras": [-22.9333, -43.1917],
  "cosme velho": [-22.941, -43.198],
  // Centro
  "centro": [-22.9035, -43.1813],
  "lapa": [-22.9129, -43.181],
  "santa teresa": [-22.925, -43.193],
  "rio comprido": [-22.923, -43.21],
  "estacio": [-22.914, -43.206],
  "catumbi": [-22.917, -43.198],
  "cidade nova": [-22.91, -43.204],
  "praca da bandeira": [-22.91, -43.216],
  "santo cristo": [-22.899, -43.199],
  "gamboa": [-22.897, -43.192],
  "saude": [-22.896, -43.183],
  "imperial de sao cristovao": [-22.902, -43.224],
  "sao cristovao": [-22.902, -43.224],
  "vasco da gama": [-22.893, -43.228],
  "caju": [-22.885, -43.215],
  "benfica": [-22.892, -43.236],
  // Grande Tijuca
  "maracana": [-22.9122, -43.23],
  "vila isabel": [-22.9167, -43.25],
  "grajau": [-22.9222, -43.2639],
  "andarai": [-22.927, -43.253],
  "tijuca": [-22.9329, -43.2386],
  "alto da boa vista": [-22.963, -43.275],
  // Grande Méier
  "sao francisco xavier": [-22.908, -43.243],
  "rocha": [-22.904, -43.25],
  "riachuelo": [-22.905, -43.257],
  "sampaio": [-22.906, -43.263],
  "engenho novo": [-22.9056, -43.2722],
  "lins de vasconcelos": [-22.916, -43.284],
  "meier": [-22.901, -43.28],
  "todos os santos": [-22.898, -43.284],
  "cachambi": [-22.8917, -43.2778],
  "engenho de dentro": [-22.8972, -43.2972],
  "agua santa": [-22.908, -43.308],
  "encantado": [-22.901, -43.304],
  "piedade": [-22.8917, -43.3083],
  "abolicao": [-22.889, -43.298],
  "pilares": [-22.883, -43.292],
  // Inhaúma, Del Castilho e Região
  "higienopolis": [-22.871, -43.262],
  "jacare": [-22.89, -43.256],
  "manguinhos": [-22.881, -43.251],
  "bonsucesso": [-22.8667, -43.2556],
  "del castilho": [-22.8778, -43.2722],
  "maria da graca": [-22.884, -43.268],
  "inhauma": [-22.873, -43.286],
  "engenho da rainha": [-22.867, -43.298],
  "tomas coelho": [-22.865, -43.308],
  "cavalcanti": [-22.869, -43.316],
  // Madureira e Cascadura
  "cascadura": [-22.8806, -43.325],
  "madureira": [-22.876, -43.336],
  "campinho": [-22.8861, -43.3444],
  "quintino bocaiuva": [-22.888, -43.318],
  "oswaldo cruz": [-22.869, -43.349],
  "bento ribeiro": [-22.868, -43.36],
  "marechal hermes": [-22.862, -43.372],
  "turiacu": [-22.864, -43.339],
  "vaz lobo": [-22.858, -43.326],
  "rocha miranda": [-22.8528, -43.3472],
  "honorio gurgel": [-22.846, -43.355],
  // Leopoldina e Penha
  "ramos": [-22.8556, -43.2556],
  "olaria": [-22.8444, -43.2667],
  "penha": [-22.8444, -43.2778],
  "penha circular": [-22.8361, -43.2889],
  "bras de pina": [-22.8278, -43.3],
  "braz de pina": [-22.8278, -43.3],
  "cordovil": [-22.8278, -43.3111],
  "parada de lucas": [-22.8194, -43.3167],
  "vigario geral": [-22.8139, -43.3111],
  "jardim america": [-22.812, -43.322],
  "vila da penha": [-22.846, -43.31],
  "vila kosmos": [-22.853, -43.307],
  "vicente de carvalho": [-22.855, -43.312],
  "iraja": [-22.8333, -43.3278],
  "vista alegre": [-22.834, -43.313],
  "colegio": [-22.845, -43.335],
  "coelho neto": [-22.8333, -43.3444],
  "acari": [-22.823, -43.342],
  "costa barros": [-22.825, -43.355],
  "barros filho": [-22.836, -43.358],
  "guadalupe": [-22.839, -43.375],
  "anchieta": [-22.822, -43.398],
  "parque anchieta": [-22.832, -43.402],
  "ricardo de albuquerque": [-22.84, -43.4],
  "pavuna": [-22.8111, -43.3667],
  "parque columbia": [-22.818, -43.335],
  // Barra, Recreio e Jacarepaguá
  "barra da tijuca": [-23.0004, -43.3658],
  "joa": [-23.012, -43.292],
  "itanhanga": [-22.989, -43.306],
  "recreio dos bandeirantes": [-23.018, -43.465],
  "recreio": [-23.018, -43.465],
  "vargem pequena": [-22.9972, -43.45],
  "vargem grande": [-23.0083, -43.4917],
  "camorim": [-22.9778, -43.4111],
  "curicica": [-22.9694, -43.3861],
  "cidade de deus": [-22.948, -43.362],
  "jacarepagua": [-22.968, -43.34],
  "gardenia azul": [-22.962, -43.348],
  "anil": [-22.9583, -43.3361],
  "freguesia (jacarepagua)": [-22.9417, -43.3417],
  "freguesia jacarepagua": [-22.9417, -43.3417],
  "freguesia": [-22.9417, -43.3417],
  "pechincha": [-22.9333, -43.3583],
  "tanque": [-22.919, -43.357],
  "taquara": [-22.9222, -43.3694],
  "praca seca": [-22.8944, -43.3556],
  "vila valqueire": [-22.8889, -43.3667],
  // Ilha do Governador e Paquetá
  "ilha do governador": [-22.813, -43.201],
  "jardim guanabara": [-22.8058, -43.2042],
  "jardim carioca": [-22.809, -43.209],
  "cacuia": [-22.817, -43.195],
  "cocota": [-22.819, -43.189],
  "taua": [-22.803, -43.186],
  "bancarios": [-22.796, -43.178],
  "freguesia (ilha do governador)": [-22.792, -43.171],
  "freguesia ilha": [-22.792, -43.171],
  "freg n s d ajuda": [-22.792, -43.171],
  "galeao": [-22.815, -43.242],
  "portuguesa": [-22.802, -43.208],
  "monero": [-22.798, -43.202],
  "pitangueiras": [-22.822, -43.184],
  "zumbi": [-22.824, -43.179],
  "ribeira": [-22.826, -43.174],
  "paqueta": [-22.76, -43.107],
  // Zona Oeste - Bangu, Campo Grande, Santa Cruz
  "deodoro": [-22.856, -43.378],
  "vila militar": [-22.863, -43.403],
  "magalhaes bastos": [-22.873, -43.418],
  "realengo": [-22.8789, -43.4328],
  "padre miguel": [-22.878, -43.448],
  "bangu": [-22.8753, -43.4678],
  "senador camara": [-22.884, -43.492],
  "santissimo": [-22.891, -43.521],
  "senador vasconcelos": [-22.898, -43.535],
  "campo grande": [-22.9036, -43.5583],
  "inhoaiba": [-22.908, -43.585],
  ...
  "ilha de guaratiba": [-23.011, -43.558],
  "pedra de guaratiba": [-22.997, -43.633],
  "barra de guaratiba": [-23.067, -43.565],
  "grumari": [-23.048, -43.525],
  "jardim sulacap": [-22.889, -43.398],
  "sulacap": [-22.889, -43.398]
};
var NITEROI_BAIRROS = {
  "centro": [-22.8833, -43.1167],
  "icarai": [-22.9064, -43.1111],
  "inga": [-22.903, -43.125],
  "santa rosa": [-22.9028, -43.1],
  "sao francisco": [-22.9222, -43.0944],
  "charitas": [-22.9306, -43.0972],
  "jurujuba": [-22.939, -43.108],
  "fonseca": [-22.88, -43.089],
  "barreto": [-22.868, -43.106],
  "itaipu": [-22.9667, -43.0444],
  "piratininga": [-22.9556, -43.0667],
  "camboinhas": [-22.9611, -43.0556],
  "itacoatiara": [-22.973, -43.031],
  "pendotiba": [-22.915, -43.062],
  "badu": [-22.922, -43.054],
  "mataruna": [-22.918, -43.058]
};
var JUIZ_DE_FORA_BAIRROS = {
  "centro": [-21.761, -43.3486],
  "sao mateus": [-21.7705, -43.3545],
  "cascatinha": [-21.782, -43.358],
  "granbery": [-21.767, -43.344],
  "alto dos passos": [-21.773, -43.35],
  "santa helena": [-21.755, -43.351],
  "bom pastor": [-21.776, -43.346],
  "paineiras": [-21.768, -43.338],
  "santa luzia": [-21.792, -43.345],
  "benfica": [-21.688, -43.432],
  "poco rico": [-21.768, -43.342],
  "grajau": [-21.748, -43.345],
  "manoel honorio": [-21.743, -43.341],
  "bairu": [-21.738, -43.344],
  "progresso": [-21.752, -43.332],
  "sao pedro": [-21.772, -43.375],
  "aeroporto": [-21.792, -43.385],
  "marilandia": [-21.785, -43.392],
  "estrela sul": [-21.781, -43.364],
  "teixeiras": [-21.789, -43.368],
  "barbosa lage": [-21.712, -43.408],
  "industrial": [-21.705, -43.415],
  "monte castelo": [-21.722, -43.378],
  "fabrica": [-21.749, -43.352]
};

// auctionPipeline/location.ts
var normalize = (s) => (s || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
var catalogs = { "rio de janeiro": Object.keys(RIO_DE_JANEIRO_BAIRROS), "niteroi": Object.keys(NITEROI_BAIRROS), "juiz de fora": Object.keys(JUIZ_DE_FORA_BAIRROS) };
var extra = {};
function configureNeighborhoods(rows) {
  extra = {};
  for (const r of rows) if (r.city && r.neighborhood) {
    const k = normalize(r.city);
    extra[k] ||= [];
    if (!extra[k].includes(r.neighborhood)) extra[k].push(r.neighborhood);
  }
}
var distance = (a, b) => {
...
function neighborhoodEvidence(input) {
  ...
  const text = [input.title, input.address].filter(Boolean).join(" ");
  const declared = input.description?.match(/bairro\s*[:\-]\s*([^\n,;]+)/i)?.[1] || "";
  const candidates = [input.neighborhood || "", declared].map((value) => value.replace(/^bairro\s*[:\-]?\s*/i, ""));
  for (const value of candidates) {
    const exact = names.find((n) => normalize(n) === normalize(value));
    if (exact) return { value: exact, method: "enum-exato" };
  }
  ...
  for (const value of candidates.filter((v) => normalize(v).length >= 6)) {
    const hits2 = names.filter((n) => distance(normalize(n), normalize(value)) <= 1);
    if (hits2.length === 1) return { value: hits2[0], method: "enum-fuzzy-distancia-1" };
  }
  return void 0;
}
function internalCep(cep) {
  try {
    const db = JSON.parse(import_node_fs2.default.readFileSync("cep_cache.json", "utf8"));
    const row = db[cep.replace(/\D/g, "")];
    return row;
  } catch {
    return void 0;
  }
}
function enrichLocation(draft) {
  const direct = neighborhoodEvidence(draft);
  if (direct) return { ...draft, neighborhood: direct.value };
  const text = [draft.address, draft.description].filter(Boolean).join(" ");
  const cep = text.match(/\b(\d{5})-?(\d{3})\b/);
  const row = cep ? internalCep(cep[1] + cep[2]) : void 0;
  if (row && normalize(row.cidade || "") === normalize(draft.city) && (!draft.state || row.uf === draft.state)) {
    const enriched = { ...draft, neighborhood: row.bairro, address: draft.address || row.logradouro };
    const verified = neighborhoodEvidence(enriched);
    if (verified) return { ...enriched, neighborhood: verified.value };
  }
  return { ...draft, neighborhood: "" };
}

// auctionPipeline/lifecycle.ts
var cheerio = __toESM(require("cheerio"), 1);
var dayEnd = (value) => /^\d{4}-\d{2}-\d{2}$/.test(value) ? Date.parse(value + "T23:59:59-03:00") : Date.parse(value);
var expiredRounds = (dates, saleMode = "", now = (/* @__PURE__ */ new Date()).toISOString()) => {
...
function assessAvailability(previous, url, httpStatus, html, now = (/* @__PURE__ */ new Date()).toISOString(), context = {}) {
  if (previous?.url !== url) previous = void 0;
  const base = { url, checkedAt: now };
  const finish = (status, reason) => ({ ...base, state: status === "expirado_link_invalido" ? "removed" : "closed", status, reason });
  if (httpStatus === 404 || httpStatus === 410) return finish("expirado_link_invalido", "HTTP " + httpStatus + ": an\xFAncio indispon\xEDvel");
  if (context.finalUrl) {
    try {
      const original = new URL(url), final = new URL(context.finalUrl);
      if (original.pathname !== "/" && /^\/(?:index\.(?:html?|php|asp))?$/i.test(final.pathname) && !final.search) return finish("expirado_link_invalido", "Detalhe redirecionado para a p\xE1gina inicial");
    } catch {
    }
  }
  if (httpStatus !== 200 || /Radware Bot Manager|<title>Just a moment|captcha|Access Denied/i.test(html)) return { ...previous, ...base, state: previous?.state || "unknown", reason: "Consulta bloqueada ou indispon\xEDvel; HTTP " + httpStatus };
  ...
  if (/^(?:im[oó]vel|lote|an[uú]ncio|p[aá]gina)\s+n[aã]o\s+encontrad[oa][.!]?$/i.test(heading)) return finish("expirado_link_invalido", "P\xE1gina oficial informa an\xFAncio n\xE3o encontrado");
  ...
  $("span,strong,b,h2,h3,p,div").each((_, e) => {
    if ($(e).children().length === 0) {
      const t = $(e).text().replace(/\s+/g, " ").trim();
      if (t.length < 100) statusText.push(t);
    }
  });
  if (statusText.some((t) => closed.test(t))) return finish("finalizado", "Encerramento expl\xEDcito no DOM do lote");
  const extra2 = [...$("body").text().matchAll(/(?:3[ºªo°]|terceir[oa]|nova)\s*(?:pra[cç]a|leil[aã]o|rodada)[^\d]{0,60}(\d{1,2})\/(\d{1,2})\/(20\d{2})/gi)].map((m) => `${m[3]}-${m[2].padStart(2, "0")}-${m[1].padStart(2, "0")}`);
  if (expiredRounds([...context.dates || [], ...extra2], context.saleMode, now)) return finish("expirado_por_data", "Todas as rodadas publicadas transcorreram, sem nova rodada futura");
  if (statusText.some((t) => /^(?:(?:lote|leil[aã]o)\s+)?(?:aberto(?: para lances)?|em andamento|dispon[ií]vel|liberado para lance)$/i.test(t))) return { ...base, state: "active", status: "ativo", reason: "Disponibilidade expl\xEDcita na fonte" };
  return { ...previous, ...base, state: previous?.state || "unknown", reason: "P\xE1gina acess\xEDvel; situa\xE7\xE3o n\xE3o confirmada" };
}
var isRetired = (a) => a.availability?.state === "closed" || a.availability?.state === "removed";
...
var normalize2 = (text) => text.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/\s+/g, " ").trim();
...
var patterns = new Map(AUCTION_STATES.map((state) => {
  const names = rowsByState.get(state).map((row) => normalize2(row.city)).sort((a, b) => b.length - a.length);
...
function sourceAuctionLocation(text, state) {
  ...
  for (const uf of state ? [state] : AUCTION_STATES) {
    ...
    for (const match of normalized.matchAll(pattern)) {
      const row = rowsByState.get(uf).find((row2) => normalize2(row2.city) === match[1]);
...
function declaredAuctionLocation(property) {
  const sourceText = (value) => (value || "").replace(/\bcampo dos goytacazes\b/gi, "Campos dos Goytacazes");
...
function validateDraft(input, options = {}) {
  const draft = enrichLocation(input);
  let html = "";
  if (draft.sourceEvidencePath) {
    try {
      html = import_node_fs3.default.readFileSync(draft.sourceEvidencePath, "utf8");
    } catch {
    }
  }
  const visible = (0, import_cheerio.load)(html);
  visible('script,style,nav,footer,aside,[hidden],[class*="related"],[class*="recommend"]').remove();
  ...
  const life = draft.sourceAvailability || assessAvailability(void 0, draft.auctionLink, 200, html, options.now, { dates: [draft.firstAuctionDate, draft.secondAuctionDate, draft.thirdAuctionDate, draft.auctionDate].filter(Boolean), saleMode: draft.saleMode });
  const dated = assessAvailability(life, draft.auctionLink, 200, html, options.now, { dates: [draft.firstAuctionDate, draft.secondAuctionDate, draft.thirdAuctionDate, draft.auctionDate].filter(Boolean), saleMode: draft.saleMode });
  if (draft.sourceClosed || isRetired({ availability: dated })) {
    const retired = draft.sourceClosed && !isRetired({ availability: dated }) ? { ...dated, state: "closed", status: "finalizado", reason: "Encerramento confirmado pelo adaptador oficial" } : dated;
  ...
  if (options.persist !== false) {
    if (reasons.length) quarantine(draft, reasons, html);
    else resolveQuarantine(draft.auctionLink);
  }
  return { draft: { ...draft, sourceAvailability: dated }, accepted: !reasons.length, retired: false, reasons };
}
var catalogEligible = (a) => !isRetired(a) && a.ingestionStatus !== "quarentena_extracao" && Number(a.auctionPrice) > 0 && a.priceVerified === true && !!a.neighborhood && !/^(?:n[aã]o informado|desconhecido|a confirmar)$/i.test(a.neighborhood);
...
function auditOfficialArea(input) {
  ...
  const amount = "(\\d+(?:[.,]\\d+)*)";
  const unit = "(?:m[\xB22]|m\\.q\\.|metros?\\s+quadrados?|ha|hectares?)";
  ...
  if (/^https:\/\/venda-imoveis\.caixa\.gov\.br\//i.test(input.url)) {
    for (const m of text.matchAll(new RegExp(amount + "\\s+de\\s+(" + label + ")", "gi"))) add(numeric(m[1]), kind(m[2]), m[0]);
  }
  ...
  for (const m of text.matchAll(new RegExp("(?:apartamento|apto\\.?|casa|sala|unidade(?:\\s+aut[o\xF4]noma)?)\\s+com\\s+(?:aproximadamente\\s+)?" + amount + "\\s*(m[\xB22]|m\\.q\\.|metros?\\s+quadrados?)", "gi"))) add(numeric(m[1]), "unspecified", m[0]);
  ...
  const ranks = land ? ["land", "total", "unspecified", "structured", "title"] : ["private", "usable", "built", "structured", "unspecified", "title", "total"];
  let selected;
  let conflict = false;
  ...
  if (selected?.kind === "total" && !land) selected = { ...selected, approximate: true };
...
// auctionIdentity.ts
var normalize3 = (v) => v.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
var token = (v) => v.replace(/^0+(?=\d)/, "");
function propertyIdentity(address, city, state, type) {
  if (!address || !city || !state) return null;
  const text = normalize3(address).replace(/(\d)\.(?=\d{3}\b)/g, "$1");
  const number = [/\b(?:numero|num|n)[ºo°.]*\s*[:.-]?\s*(\d+[a-z]?)\b/i.exec(text), /,\s*(\d+[a-z]?)\b/.exec(text)].filter((m) => Boolean(m)).sort((a, b) => a.index - b.index)[0];
  ...
  const units = specs.flatMap(([kind, re]) => {
    const m = rest.match(re);
    return m ? [kind + "-" + token(m[1])] : [];
  });
  if (/apartamento|comercial|cobertura|sala/i.test(type || "") && !units.some((u) => /^(ap|sl)-/.test(u))) return null;
  return [normalize3(city), normalize3(state), street, token(number[1]), ...units].join("|");
}

// auctionPipeline/aggregation.ts
function aggregateAuctions(auctions, canonicalize) {
  const sourceKey = (url) => {
    try {
      const u = new URL(url);
      const id = u.pathname.match(/[-/]([jx]\d+)\/?$/i);
      if (/(^|\.)megaleiloes\.com\.br$/i.test(u.hostname) && id) return "megaleiloes|" + id[1].toLowerCase();
    } catch {
    }
    return canonicalize(url);
  };
  const keys = /* @__PURE__ */ new Map(), links = /* @__PURE__ */ new Map();
  const groups = [];
  for (const a of auctions) {
    const urls = [a.auctionLink, ...a.sourceLinks || []].filter(Boolean).map((u) => sourceKey(u));
    ...
    let group = urls.map((u) => links.get(u)).find(Boolean);
    const candidate = key ? keys.get(key) : void 0;
    const differentContract = candidate?.some((b) => b.id.startsWith("auc-caixa-") && a.id.startsWith("auc-caixa-") && b.id !== a.id);
    group ||= differentContract ? void 0 : candidate;
    if (!group) {
      group = [];
      groups.push(group);
    }
    group.push(a);
    for (const u of urls) links.set(u, group);
    if (key && !keys.has(key)) keys.set(key, group);
  }
  const timestamp = (a) => a.lastSyncedAt || a.availability?.checkedAt || "";
  return groups.map((originalGroup) => {
    const records = /* @__PURE__ */ new Map();
    for (const a of originalGroup) {
      for (const r of a.sourceRecords || []) {
        const key = canonicalize(r.auctionLink || r.id);
        if (!records.has(key)) records.set(key, r);
      }
      const { sourceRecords, offers: offers2, ...snapshot } = a;
      records.set(canonicalize(a.auctionLink || a.id), snapshot);
    }
    const group = [...records.values()].map((a) => {
      if (!isRetired(a) && a.ingestionStatus !== "quarentena_extracao" && expiredRounds([a.firstAuctionDate, a.secondAuctionDate, a.thirdAuctionDate, a.auctionDate].filter(Boolean), a.saleMode)) return { ...a, ingestionStatus: "expirado_por_data", availability: { state: "closed", status: "expirado_por_data", url: a.auctionLink || "", checkedAt: (/* @__PURE__ */ new Date()).toISOString(), reason: "Todas as rodadas transcorreram" } };
      return a;
    });
    const live = group.filter((a) => !isRetired(a) && a.ingestionStatus !== "quarentena_extracao");
    const source = (live.length ? live : group).slice().sort((a, b) => timestamp(b).localeCompare(timestamp(a)))[0];
    ...
    const bid = offers[0] || source;
    const urls = [...new Set(group.flatMap((a) => [a.auctionLink, ...a.sourceLinks || []]).filter(Boolean).map((u) => canonicalize(u)))];
    ...
    return {
      ...source,
      id: originalGroup[0].id,
      sourceRecords: group.length > 1 ? group : void 0,
      canonicalPropertyId: (0, import_node_crypto2.createHash)("sha256").update(identity).digest("hex"),
      auctionLink: bid.auctionLink,
      auctionPrice: bid.auctionPrice,
      auctionDate: bid.auctionDate,
      firstAuctionDate: bid.firstAuctionDate,
      secondAuctionDate: bid.secondAuctionDate,
      thirdAuctionDate: bid.thirdAuctionDate,
      firstAuctionPrice: bid.firstAuctionPrice,
      secondAuctionPrice: bid.secondAuctionPrice,
      priceVerified: bid.priceVerified,
      availability: bid.availability,
      sourceLinks: urls,
      links_adicionais: urls.filter((u) => u !== canonicalize(bid.auctionLink || "")),
      saved: group.some((a) => a.saved),
...
function extractAuctionDates(text) {
  ...
  }).map((match) => `${match[3].length === 2 ? "20" + match[3] : match[3]}-${match[2].padStart(2, "0")}-${match[1].padStart(2, "0")}`).filter((date) => {
...
function extractDisplayedCurrentBid(text) {
...
function extractAuctionRoundsAndPrices(text, todayParam) {
  ...
  const unique = (values) => {
    const v = [...new Set(values.filter((n) => n > 0))];
    return v.length === 1 ? v[0] : 0;
  };
  const bids = (block) => [...block.matchAll(/(?:lance\s*(?:inicial|m[ií]nimo)|valor\s*(?:inicial|m[ií]nimo)(?:\s*para\s*proposta)?)\s*:?\s*R\$\s*([\d.]+(?:,\d{2})?)/gi)].map((m) => parseBrazilianMoney(m[1]));
  const round = (n) => {
    const entries = markers.filter((m) => Number(m[1] || (/primeir/i.test(m[2]) ? 1 : 2)) === n).map((m) => {
      const index = markers.indexOf(m);
      const block = text.slice(m.index + m[0].length, markers[index + 1]?.index ?? text.length).slice(0, 350);
      const date = block.match(/^\s*:?\s*(?:dia\s*)?(\d{1,2}\/\d{1,2}\/\d{2,4})/i)?.[1];
      const labelled = bids(block);
      const bare = block.match(/^\s*:?\s*(?:\d{1,2}\/\d{1,2}\/\d{2,4}(?:\s+(?:às\s*)?\d{1,2}:\d{2})?\s*[-–:]?\s*)?R\$\s*([\d.]+(?:,\d{2})?)/i);
      return { price: unique(labelled.length ? labelled : bare ? [parseBrazilianMoney(bare[1])] : []), date: date ? normalizeDateStr(date) : void 0 };
    });
    const dates = [...new Set(entries.map((e) => e.date).filter(Boolean))];
    return { price: unique(entries.map((e) => e.price)) || void 0, date: dates.length === 1 ? dates[0] : void 0 };
  };
  const one = round(1), two = round(2);
  ...
  if (firstAuctionDate && firstAuctionDate >= today) {
    activePrice = firstAuctionPrice || genericPrice;
    activeDate = firstAuctionDate;
  } else if (secondAuctionDate && secondAuctionDate >= today) {
    activePrice = secondAuctionPrice || genericPrice;
    activeDate = secondAuctionDate;
  } else if (!firstAuctionDate && !secondAuctionDate) {
...
${extracted}`;
    ...
    return parseOfficialLotDetail(draft, { ...detailData, officialFinancials, sourceEvidencePath, availability: assessAvailability(void 0, detailPage.url(), 200, officialHtml) }, detailPage.url(), matriculaText, matriculaUrl);
  } catch (err) {
...
function parseOfficialLotDetail(draft, detailData, detailUrl, matriculaText = "", matriculaUrl) {
  ...
  return {
    ...
    firstAuctionDate,
    secondAuctionDate,
    auctionDate: officialFin?.auctionDate || roundAnalysis.activeDate || [firstAuctionDate, secondAuctionDate].filter((date) => Boolean(date) && date >= today).sort()[0] || firstAuctionDate || dates.first || "",
...
function reconcileAuctionDrafts(allDrafts, targetType, state, city, existingAuctions, recalculateFn, writeAudit = true, quarantineEnabled = writeAudit) {
  ...
  for (let draft of allDrafts) {
    ...
    if (!validation.accepted && !validation.retired) {
      ...
      pendingReview.push({ draft, reason: validation.reasons.join(",") });
      continue;
    }
    const declared = declaredAuctionLocation(draft);
...
${draft.description || ""}`)) {
    ...
    if (draft.sourceClosed) {
      ...
      if (saved && canonicalAuctionLink(saved.auctionLink || "") === canonicalAuctionLink(draft.auctionLink)) {
        saved.availability = draft.sourceAvailability || { state: "closed", status: "finalizado", url: draft.auctionLink, checkedAt: (/* @__PURE__ */ new Date()).toISOString(), reason: "Encerramento confirmado pela coleta oficial" };
        updated++;
      }
    ...
    if (existing) {
      ...
      Object.assign(existing, recalculateFn({
        ...
        availability: draft.sourceAvailability?.state === "active" ? draft.sourceAvailability : existing.availability,
        ...
        thirdAuctionDate: draft.thirdAuctionDate,
      ...
      if (canonicalAuctionLink(previousOffer.auctionLink || "") !== draft.auctionLink) Object.assign(existing, recalculateFn({ ...aggregateAuctions([previousOffer, { ...existing, sourceRecords: void 0 }], canonicalAuctionLink)[0], id: existing.id }));
    ...
    existingLinks.add(draft.auctionLink);
    const baseId = `auc-${draft.portalId}-${(0, import_node_crypto3.createHash)("sha256").update(draft.auctionLink).digest("hex").slice(0, 20)}`;
...
async function collectBankApi(id, dir, onPage) {
  ...
  for (let page = 1; page <= maximum; page++) {
    ...
    import_node_fs4.default.writeFileSync(import_node_path2.default.join(dir, `${id}-api-${page}.json`), JSON.stringify(body));
...
async function collectSoldApi(dir, onPage) {
  ...
  for (let page = 1; seen.size < total; page++) {
    ...
    const body = await response.json();
    import_node_fs4.default.writeFileSync(import_node_path2.default.join(dir, `sold-api-${page}.json`), JSON.stringify(body));
...
async function collectRioLinks(dir, onPage) {
  ...
  import_node_fs4.default.writeFileSync(import_node_path2.default.join(dir, "rioleiloes-events.json"), JSON.stringify(inventory));
  ...
  for (const event of inventory.items) {
    ...
    import_node_fs4.default.writeFileSync(import_node_path2.default.join(dir, `rioleiloes-event-${event.id}.json`), JSON.stringify(lots));
...
async function collectPestanaApi(dir, onPage) {
  ...
  import_node_fs4.default.writeFileSync(import_node_path2.default.join(dir, "pestana-events.json"), JSON.stringify(events));
  ...
  for (let start2 = 0; start2 < ids.length; start2 += 80) {
    ...
    import_node_fs4.default.writeFileSync(import_node_path2.default.join(dir, `pestana-cards-${start2}.json`), JSON.stringify(cards));
    ...
    if (targets.length) {
      ...
      import_node_fs4.default.writeFileSync(import_node_path2.default.join(dir, `pestana-details-${start2}.json`), JSON.stringify(details));
      ...
      for (const lot of details) {
        ...
        const text = cheerio2.load(description).text();
...
function parseSourcePage(html, url) {
  ...
  if (lotData?.descricao) {
    const description = cheerio3.load(lotData.descricao).text();
  ...
  if (/\/item\/\d+\/detalhes/.test(new URL(url).pathname)) {
    ...
    if (displayed) {
      const value = Number(displayed[1].replace(/\./g, "").replace(",", "."));
      if (value > 0) financials = { ...financials, auctionPrice: value, priceVerified: true };
    }
  }
  if (new URL(url).hostname.replace(/^www\./, "") === "paulobotelholeiloeiro.com.br" && /\/leiloes\/.+\/\d+$/.test(new URL(url).pathname)) {
    const money = (v) => v ? Number(v.replace(/\./g, "").replace(",", ".")) : 0;
    const date = (v) => v ? v.split("/").reverse().join("-") : void 0;
  ...
  return { availability: assessAvailability(void 0, url, 200, html), links, text, title: lotData?.titulo || title, image, structuredAddresses, structuredSizes, documentLinks, officialFinancials: financials, lotDescription: lotData?.descricao ? cheerio3.load(lotData.descricao).text() : "" };
}
async function runListedPortalSync(reason, onDrafts, options = {}) {
  ...
  const dir = import_node_path3.default.join("sync-audits", "runs", report.id);
  import_node_fs5.default.mkdirSync(dir, { recursive: true });
  const save = () => {
    import_node_fs5.default.writeFileSync(import_node_path3.default.join(dir, "report.json"), JSON.stringify(report, null, 2));
    import_node_fs5.default.writeFileSync("sync-audits/latest-listed-sync.json", JSON.stringify(report, null, 2));
  };
  save();
  ...
  const load5 = async (url, render = false) => {
  ...
  const worker = async () => {
    while (cursor < configs.length) {
      ...
      const flush = async () => {
        ...
        import_node_fs5.default.appendFileSync(import_node_path3.default.join(dir, config.id + ".jsonl"), rows.map((r) => JSON.stringify(r)).join("\n") + "\n");
        const audits = rows.filter((r) => r.areaAudit).map((r) => JSON.stringify({ source: config.id, url: r.auctionLink, ...r.areaAudit }));
        if (audits.length) import_node_fs5.default.appendFileSync(import_node_path3.default.join(dir, "area-audit.jsonl"), audits.join("\n") + "\n");
      ...
      try {
        if (["emgea", "vitrinebradesco", "pestana"].includes(config.id)) {
          const onPage = async (rows, total) => {
            ...
            batches.push(...rows.map((row) => ({ ...row, sourceEvidencePath: preserveSource(row.auctionLink, "<pre>" + JSON.stringify(row) + "</pre>") })));
            await flush();
            save();
          };
        ...
        while (listingQueue.length) {
          ...
          try {
            let response;
            try {
              response = await load5(requested);
            } catch (error) {
              if (renderedOnce) throw error;
              renderedOnce = true;
              response = await load5(requested, true);
            }
            ...
            import_node_fs5.default.writeFileSync(import_node_path3.default.join(dir, config.id + "-listing-" + progress.pages + ".html"), response.html);
          ...
          for (const [url, listingText] of pending) {
            ...
            try {
              let response;
              try {
                response = await load5(url);
              } catch {
                response = await load5(url, true);
              }
              let page = parseSourcePage(response.html, response.url);
              progress.fetched++;
              ...
              page = { ...page, sourceEvidencePath: preserveSource(response.url, response.html) };
              ...
              if (!draft.priceVerified || !draft.addressVerified || draft.areaAudit?.status === "missing") {
                try {
                  const renderedResponse = await load5(url, true);
                  const renderedPage = parseSourcePage(renderedResponse.html, renderedResponse.url);
                  const checked = parseOfficialLotDetail(initial, { ...renderedPage, sourceEvidencePath: preserveSource(renderedResponse.url, renderedResponse.html) }, renderedResponse.url);
                  if (checked.priceVerified || checked.areaAudit?.selected || checked.addressVerified) {
                    draft = { ...checked, ...!checked.priceVerified && draft.priceVerified ? { auctionPrice: draft.auctionPrice, priceVerified: true, firstAuctionPrice: draft.firstAuctionPrice, secondAuctionPrice: draft.secondAuctionPrice } : {}, ...!checked.sizeVerified && draft.sizeVerified ? { sizeSqm: draft.sizeSqm, sizeVerified: true, areaAudit: draft.areaAudit } : {} };
                    page = renderedPage;
                  }
                } catch (error) {
              ...
              batches.push(draft);
              progress.accepted++;
              if (batches.length >= 10 || Date.now() - lastFlushAt > 15e3) await flush();
            } catch (error) {
...
var file = import_node_path4.default.join(process.cwd(), "official_property_locations.json");
var normalize4 = (s) => (s || "").normalize("NFC").trim().toLocaleLowerCase("pt-BR").replace(/\s+/g, " ");
var propertyLocationKey = (p) => JSON.stringify([normalize4(p.state), normalize4(p.city), normalize4(p.neighborhood), normalize4(p.address)]);
...
function refresh() {
  let stat;
  try {
    stat = import_node_fs6.default.statSync(file);
  } catch {
  ...
  try {
    const rows = JSON.parse(import_node_fs6.default.readFileSync(file, "utf8"));
...
function ensureOfficialLocationCoverage(properties) {
  ...
  const fingerprint = (0, import_node_crypto4.createHash)("sha256").update(JSON.stringify(inputs)).digest("hex");
  if (fingerprint === lastRefreshKey) return;
  const cache2 = import_node_path4.default.join(process.cwd(), ".cache", "map-cnefe");
  if (!import_node_fs6.default.existsSync(import_node_path4.default.join(cache2, "manifest.json"))) return;
  let config = {};
  try {
    config = JSON.parse(import_node_fs6.default.readFileSync(import_node_path4.default.join(process.cwd(), "map_sources_config.json"), "utf8"));
  } catch {
  }
  const python = process.env.PYTHON_EXECUTABLE || config.pythonExecutable || "python";
  const inputPath = import_node_path4.default.join(cache2, "refresh_targets.json");
  import_node_fs6.default.writeFileSync(inputPath, JSON.stringify(inputs));
  lastRefreshKey = fingerprint;
  refreshing = true;
  const child = (0, import_node_child_process.spawn)(python, [import_node_path4.default.join(process.cwd(), "scripts", "refresh_map_locations.py"), inputPath], {
    cwd: process.cwd(),
    windowsHide: true,
...
function checkCommunityCentroidRisk(auc) {
  ...
  for (const community of communityCentroids) {
    ...
    const distance2 = calculateDistanceKm(lat, lng, community.lat, community.lng) * 1e3;
    if (distance2 <= 350 && (!nearest || distance2 < nearest.distance)) {
      nearest = { name: community.n, faction: community.f, distance: distance2 };
    }
  }
    ...
    try {
      ...
      if (storeData.auctions) {
        ...
        storeData.auctions.forEach((a) => {
          ...
          const reason = !isAllowedTargetCity(a.city, a.state) ? "fora_das_cidades_configuradas" : actual && (normalize(actual.city) !== normalize(a.city) || actual.state !== a.state) ? "localidade_divergente_do_detalhe" : /^(auc-port-|auc-jud-|auc-ext-)/.test(a.id || "") ? "registro_legado_sem_evidencia" : !realEstate && nonPropTerms.some((term) => t.includes(term)) ? "lote_nao_imobiliario" : "";
          if (reason) {
            a.ingestionStatus = "quarentena_extracao";
            quarantine(a, [reason]);
          }
        });
        storeData.auctions.forEach((a) => {
...
var availabilityRunning = null;
var catalogRevision = Date.now();
function startAvailabilityCheck() {
  ...
  availabilityRunning = (async () => {
    ...
    const rows = store.auctions.flatMap((parent) => {
      ...
      const { sourceRecords, offers, ...primary } = parent;
      sources.set(canonicalAuctionLink(parent.auctionLink || ""), primary);
      if (sources.size > 1) parent.sourceRecords = [...sources.values()];
      return [...sources.values()].filter((a) => {
        try {
          return allowed.has(new URL(a.auctionLink || "").hostname.replace(/^www\./, ""));
        } catch {
          return false;
        }
      }).map((a) => ({ parentId: parent.id, ...a }));
    });
    let cursor = 0;
    await Promise.all(Array.from({ length: 4 }, async () => {
      while (cursor < rows.length) {
        const snapshot = rows[cursor++], url = snapshot.auctionLink;
        ...
        const parent = store.auctions.find((a) => a.id === snapshot.parentId);
        if (!parent) continue;
        const current = parent.sourceRecords?.find((a) => canonicalAuctionLink(a.auctionLink || "") === canonicalAuctionLink(url)) || parent;
        if (current.auctionLink !== url) continue;
        current.availability = assessAvailability(current.availability, url, status, html, void 0, { finalUrl });
        if (isRetired(current)) {
          ...
          if (page.text.length > 100) {
            ...
            if (review.retired) {
              ...
              current.ingestionStatus = "quarentena_extracao";
              current.precisa_revisao = true;
              current.liquidityScore = 1;
            }
          }
        }
    ...
    for (const parent of store.auctions) {
      if (parent.sourceRecords?.length) {
        const merged = deduplicateAuctions(parent.sourceRecords)[0];
        Object.assign(parent, recalculateAuction({ ...merged, id: parent.id, saved: parent.saved }, store.itbiTransactions));
      }
    }
    saveStore(store);
  })().catch((e) => console.error("[Availability]", e)).finally(() => {
    availabilityRunning = null;
  });
}
function readListedSyncStatus() {
  try {
    ...
    return { catalogRevision, availabilityRunning: Boolean(availabilityRunning) };
  }
}
...
app.post("/api/auth/register", (req, res) => {
  ...
  const token2 = import_crypto.default.randomBytes(32).toString("hex");
  const expiresAt = isFirstUser ? Date.now() + 365 * 24 * 60 * 60 * 1e3 : newUser.licenseExpiresAt || Date.now() + licenseDuration;
  store.sessions.push({ token: token2, userId: newUser.id, expiresAt });
  saveStore(store);
  const { passwordHash: _, salt: __, ...userPublic } = newUser;
  res.json({ token: token2, user: userPublic });
});
app.post("/api/auth/login", (req, res) => {
  ...
  store.sessions.push({ token: token2, userId: user.id, expiresAt });
  saveStore(store);
...
app.post("/api/auth/renew-license", (req, res) => {
  ...
  const token2 = import_crypto.default.randomBytes(32).toString("hex");
  store.sessions.push({ token: token2, userId: user.id, expiresAt: user.licenseExpiresAt });
  saveStore(store);
  const { passwordHash: _, salt: __, ...userPublic } = user;
  res.json({ success: true, message: "Licen\xE7a renovada com sucesso por mais 7 dias!", token: token2, user: userPublic });
});
app.post("/api/auth/logout", authMiddleware, (req, res) => {
  const authHeader = req.headers.authorization;
  const token2 = authHeader?.split(" ")[1];
  store.sessions = store.sessions.filter((s) => s.token !== token2);
  saveStore(store);
...
app.get("/api/auctions/bbox", (req, res) => {
  ...
  if (authHeader && authHeader.startsWith("Bearer ")) {
    ...
    const session = store.sessions.find((s) => s.token === token2 && new Date(s.expiresAt) > /* @__PURE__ */ new Date());
    if (session) requestUserId = session.userId;
  }
  const userAuctions = deduplicateAuctions(store.auctions.filter(
    (a) => catalogEligible(a) && (!a.userId || a.userId === requestUserId || a.origin === "caixa_radar" || a.origin === "caixa" || a.origin === "judicial" || a.origin === "extrajudicial" || a.origin === "portal") && isAllowedTargetCity(a.city, a.state)
